...
@Component({selector:'app-audit-log',standalone:true,imports:[CommonModule,RouterModule,MatCardModule,MatButtonModule,MatIconModule,VndPipe,StatusViPipe],
...
<a routerLink="/admin/config" routerLinkActive="act"><mat-icon>settings</mat-icon>Cài đặt</a></nav></div><div class="am"><h2>Audit Log</h2><mat-card class="tc2"><div class="th"><span>Thời gian</span><span>User</span><span>Action</span><span>Entity</span><span>Chi tiết</span><span>IP</span></div>
<div *ngFor="let l of data()" class="tr"><span>{{l.timestamp|date:'dd/MM HH:mm'}}</span><span><b>{{l.username}}</b><br/><small>{{l.role}}</small></span>
<span><small class="at" [class]="l.action.toLowerCase()">{{l.action}}</small></span><span>{{l.entity}}</span><span class="desc">{{l.details}}</span><span>{{l.ipAddress}}</span></div></mat-card>
<style>.tc2{padding:0!important;border-radius:12px!important;overflow:hidden}.th,.tr{display:grid;grid-template-columns:.8fr 1fr .7fr .7fr 1.5fr .8fr;padding:10px 14px;font-size:12px;align-items:center}.th{background:#f5f7fa;font-weight:600;color:#888}.tr{border-bottom:1px solid #f5f5f5;b{font-size:12px}small{font-size:10px;color:#888}}.desc{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.at{padding:2px 8px;border-radius:10px;font-weight:600;font-size:10px;&.create{background:#e8f5e9;color:#2e7d32}&.update,.config{background:#e3f2fd;color:#1565c0}&.approve{background:#fff3e0;color:#e65100}&.submit{background:#ede7f6;color:#4527a0}&.login{background:#f5f5f5;color:#757575}&.payment{background:#e0f7fa;color:#00695c}&.query{background:#fce4ec;color:#c62828}}</style></div></div>`,
...
export class AuditlogComponent implements OnInit{
  ...
  ngOnInit(){this.api.getAuditLogs().subscribe(l=>this.data.set(l));}
}