...
@Component({selector:'app-partner-management',standalone:true,imports:[CommonModule,RouterModule,MatCardModule,MatButtonModule,MatIconModule,VndPipe,StatusViPipe],
...
<a routerLink="/admin/config" routerLinkActive="act"><mat-icon>settings</mat-icon>Cài đặt</a></nav></div><div class="am"><h2>Quản lý đối tác</h2><div class="pg"><mat-card *ngFor="let p of data()" class="pc" appearance="outlined">
<div class="pr"><mat-icon class="pi2" [class]="p.type.toLowerCase()">{{p.type==='HOSPITAL'?'local_hospital':p.type==='GARAGE'?'car_repair':'account_balance'}}</mat-icon>
<div class="pinfo"><h3>{{p.name}}</h3><p>{{p.type|statusVi}} · {{p.tier|statusVi}} · {{p.address}}</p></div>
<div class="ps"><span class="db" *ngIf="p.directBilling">Direct Billing</span></div></div>
<div class="pd"><div><span>Claims</span><b>{{p.claimsHandled|number}}</b></div><div><span>Billing</span><b>{{p.totalBilling|vnd}}</b></div><div><span>Liên hệ</span><b>{{p.contactPerson}}</b></div></div>
</mat-card></div>
<style>.pg{display:grid;gap:12px}.pc{border-radius:12px!important;padding:18px!important}.pr{display:flex;align-items:center;gap:14px;margin-bottom:12px}.pi2{font-size:32px;width:40px;height:40px;display:flex;align-items:center;justify-content:center;border-radius:10px;color:#fff;&.hospital{background:#4CAF50}&.garage{background:#FF9800}&.bank{background:#2196F3}}.pinfo{flex:1;h3{font-size:14px;font-weight:600;margin:0}p{font-size:12px;color:#888;margin:0}}.ps{.db{padding:3px 10px;background:#e8f5e9;color:#2e7d32;border-radius:12px;font-size:11px;font-weight:600}}.pd{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;padding-top:10px;border-top:1px solid #f0f0f0;span{font-size:11px;color:#888;display:block}b{font-size:12px}}</style></div></div>`,
...
export class PartnermanagementComponent implements OnInit{
  ...
  ngOnInit(){this.api.getPartners().subscribe(p=>this.data.set(p));}
}