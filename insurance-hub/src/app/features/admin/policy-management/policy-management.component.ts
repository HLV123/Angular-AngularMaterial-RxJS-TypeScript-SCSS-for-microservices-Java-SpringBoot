...
@Component({selector:'app-policy-management',standalone:true,imports:[CommonModule,RouterModule,MatCardModule,MatButtonModule,MatIconModule,VndPipe,StatusViPipe],
...
<a routerLink="/admin/config" routerLinkActive="act"><mat-icon>settings</mat-icon>Cài đặt</a></nav></div><div class="am"><h2>Quản lý hợp đồng</h2><mat-card class="tc2"><div class="th"><span>Mã HĐ</span><span>Khách hàng</span><span>Sản phẩm</span><span>Phí</span><span>Trạng thái</span></div>
<div *ngFor="let p of data()" class="tr"><span><b>{{p.policyNumber}}</b></span><span>{{p.customerName}}</span><span>{{p.productName}}</span>
<span>{{p.premium|vnd}}</span><span><small class="st" [class]="p.status.toLowerCase()">{{p.status|statusVi}}</small></span></div></mat-card>
<style>.tc2{padding:0!important;border-radius:12px!important;overflow:hidden}.th,.tr{display:grid;grid-template-columns:1.5fr 1fr 1.2fr .8fr .8fr;padding:12px 16px;font-size:13px;align-items:center}.th{background:#f5f7fa;font-weight:600;font-size:12px;color:#888}.tr{border-bottom:1px solid #f5f5f5;b{font-size:12px}}.st{padding:2px 8px;border-radius:10px;font-size:10px;font-weight:600;&.active{background:#e8f5e9;color:#2e7d32}&.expired{background:#f5f5f5;color:#757575}&.pending_payment{background:#e3f2fd;color:#1565c0}}</style></div></div>`,
...
export class PolicymanagementComponent implements OnInit{
  ...
  ngOnInit(){this.api.getPolicies().subscribe(p=>this.data.set(p));}
}